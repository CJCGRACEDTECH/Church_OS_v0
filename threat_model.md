...
- **Highest-risk areas:** auth/JIT provisioning (`routes/auth.ts`, `middlewares/auth.ts`), admin/invitation and settings control plane (`routes/admin.ts`, `routes/settings.ts`), giving/payment/webhook logic (`routes/giving.ts`), attendance/check-in APIs (`routes/attendance.ts`, `routes/children-checkin.ts`)
- **Public vs authenticated vs admin:** health and invite acceptance endpoints are public; member routes require auth; admin and permission endpoints must enforce role and permission checks server-side
